...
function processSales($data, $upline = [], $level = 0, &$flatSales, &$commission, $rates) {
    foreach ($data as $item) {
        ...
        if (!empty($item['referrals'])) {
            processSales($item['referrals'], array_merge([$seller], $upline), $level + 1, $flatSales, $commission, $rates);
        }
    }
}

processSales($sales, [], 0, $flatSales, $commission, $commissionRates);
