...
{
    $interest = 0;
    $remaining = $balance; //30000//20000
    $lastCap = 0; //10000//20000

    foreach ($tiers as $tier) {
        $cap = $tier['up_to'] ?? $balance; //10000 //20000 //30000
        $amountInTier = min($remaining, $cap - $lastCap); //(30000,10000-0) (20000,20000-10000) //(10000,20000-null)
...
function calculateInterest($accounts, $holidaysByCountry, $taxRates, $exchangeRates)
{
    ...
    foreach ($accounts as $account) {
        ...
        // $current = $start;
        while ($start <= $end) {
            if (isBusinessDay($start, $holidaysByCountry[$country])) {
                $businessDays++;
            }
            $start->modify('+1 day');
        }
        ...
        // Compound Interest
        $final = $balance * pow(1 + $rate / $n, $n * $t); //$balance=40000, rate =0.0525, $n=365, $t=0.7095890410958904
        $interest = $final - $balance;
        
        // Tax
        $tax = $interest *  $taxRates[$country];//270.85380
        var_dump($tax);
        //finlinterest
    ...
    return ['results' => [$id => $usdFinal], 'total_tax' => round($totalTax, 2)];
}
