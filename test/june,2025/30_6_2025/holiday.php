...
function calculateInterest($account, $holidays) {
    ...
    if ($account['interest_type'] === 'compound') {
        ...
        } elseif ($compounding === 'weekly') {
            $days = $interval->days;
            $weeks = intdiv($days, 7);
            $periods = $weeks;
            $periodRate = $annualRate / 52;
        } else {
            // default fallback to annual
            $periods = 1;
            $periodRate = $annualRate;
        }

        $futureValue = $balance * pow((1 + $periodRate), $periods);
        $interest = $futureValue - $balance;
        $description = "Compound Interest (" . ucfirst($compounding) . ")";
        ...
        $description = "Simple Interest (Excl. weekends & holidays)";
    }
...
foreach ($accounts as $acc) {
    $result = calculateInterest($acc, $holidays);
    echo "Account {$result['account_id']}<br>";
    echo "{$result['description']}: {$result['interest']}<br><br>";
}
