...
function calculateCommissions($sales, &$results = [], $uplines = []) {
    foreach ($sales as $seller) {
        ...
        if (!isset($results[$id])) {
            $results[$id] = [
                ...
                'upline_commission' => 0,
                'team_sales' => 0,
                'self_sales' => 0,   // Track personal sales
                'bonus' => 0,
            ];
        }
    ...
    foreach ($results as $seller_id => &$data) {
        ...
        if ($threshold > 0) {
            $team_only_sales = $data['team_sales'] - $data['self_sales'];
            $data['bonus'] = floor($team_only_sales / $threshold) * 100;
        }
    }
...
// Output final commission total for each seller
foreach ($results as $seller_id => $data) {
    $total = $data['self_commission'] + $data['upline_commission'] + $data['bonus'];
    echo "commission of $seller_id = " . round($total, 2) . "<br>";
}
?>
